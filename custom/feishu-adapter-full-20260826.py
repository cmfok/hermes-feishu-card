...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # 2026-08-07 修复：纯内存不持久化——重启后映射为空 → 一律发新卡，
        # 避免跨会话/跨天 PATCH 到旧卡片（bug：今早内容更新到昨晚卡片）。
        self._chat_card_map: Dict[str, str] = {}
        # 卡片密封定时器（chat_id → asyncio.Task）：文本/收尾消息后延迟密封
        # 「✅ 完成」。工具消息到达会取消定时器——长工具调用期间永不误标完成。
        self._card_seal_timers: Dict[str, asyncio.Task] = {}
        # 卡片 PATCH 锁（chat_id → asyncio.Lock）：密封定时器 PATCH 与 send PATCH
        # 并发改同一卡片会触发飞书 400 → fallback 发新卡 → 卡片内容跳变丢失。
        # 串行化后无并发，2026-08-07 CM 实测「小卡片→整卡→两行字」跳变根因。
        self._card_patch_locks: Dict[str, asyncio.Lock] = {}
    ...
    async def send(
        ...
        if existing_card and isinstance(existing_card, dict) and existing_card.get("message_id"):
            if FeishuAdapter._card_exceeds_limits(existing_card):
                # 容量保护：追加次数 / 块数 / 整卡字节数任一超限 → 主动开新卡，
                # 避免无限追加触达飞书卡片上限后 PATCH 失败、旧内容断掉。
                logger.info(
                    "[Feishu] Card at capacity (%d appends, %d blocks, %d B); starting new card",
                    existing_card.get("append_count", 0),
                    len(existing_card.get("blocks", [])),
                    len(str(existing_card.get("content", "")).encode("utf-8")),
                )
                existing_card = None
            else:
                try:
                    blocks = existing_card.get("blocks", [])
                    if FeishuAdapter._is_closing_message(formatted):
                        # 收尾类消息（💾 记忆保存/后台审查通知）：不进 blocks、
                        # 不打断状态行，只重置密封定时器——回复完成后 Hermes 会
                        # 调 memory 工具收尾，它不该把「✅ 完成」打回「运行中」。
                        self._schedule_seal(chat_id, existing_card)
                        return SendResult(success=True, message_id=existing_card["message_id"])
                    is_tool = FeishuAdapter._looks_like_tool_message(formatted)
                    if is_tool:
                        # 工具消息：加入/合并工具块（同一工具连续调用合并）+
                        # 取消密封定时器（工具还在跑，绝不提前标完成）
                        self._cancel_seal_timer(chat_id)
                        if blocks and blocks[-1].get("type") == "tools" and blocks[-1].get("tool_pending"):
                            blocks[-1]["content"] += "\n\n" + formatted
                        else:
                            blocks.append({"type": "tools", "content": formatted, "expanded": True, "tool_pending": True})
                        new_status = "running"
                    else:
                        # 文本消息：收起之前的工具框 + 追加消息块；不立即标完成，
                        # 重置密封定时器——流式回复分段到达期间保持「回复中」，
                        # 整轮结束（N 秒无新消息）后才变「完成」。
                        for b in blocks:
                            if b.get("type") == "tools":
                                b["expanded"] = False
                                b.pop("tool_pending", None)
                        blocks.append({"type": "message", "text": formatted})
                        new_status = "typing"
                        self._schedule_seal(chat_id, existing_card)
                    existing_card["blocks"] = blocks
                    existing_card["content"] = str(existing_card.get("content", "")) + "\n\n" + formatted
                    existing_card["append_count"] = existing_card.get("append_count", 0) + 1
                    new_payload = FeishuAdapter._card_with_blocks(blocks, status=new_status)
                    # 直接 PATCH 原始 payload（不走 edit_message——它会重建卡片，把 blocks 丢弃）
                    # 持锁 PATCH：与密封定时器 PATCH 串行，防并发 400 导致内容跳变
                    async with self._get_patch_lock(chat_id):
                        result = await self._patch_card_raw(existing_card["message_id"], new_payload)
                    if result.success:
                        existing_card["content"] = new_payload
                        existing_card["blocks"] = blocks
                        return result
        ...
        try:
            ...
            if result.success and result.message_id:
                blocks = []
                if not FeishuAdapter._is_closing_message(formatted) and FeishuAdapter._looks_like_tool_message(formatted):
                    blocks.append({"type": "tools", "content": formatted, "expanded": True, "tool_pending": True})
                else:
                    blocks.append({"type": "message", "text": formatted})
                self._chat_card_map[chat_id] = {
                    "message_id": result.message_id,
                    "content": FeishuAdapter._card_with_blocks(blocks, status="running"),
                    "blocks": blocks,
                    "append_count": 1,
                }
            return result
    ...
    async def _dispatch_inbound_event(self, event: MessageEvent) -> None:
        ...
        # 2026-08-07 修复：用户新消息 = 新轮次 → 清掉该会话的卡片映射，
        # 下一轮回复发新卡片（避免 PATCH 到上一轮的旧卡，跨会话/跨天 bug）。
        try:
    ...
    # 密封延迟：文本/收尾消息后 N 秒无新消息 → 状态行变「✅ 完成」。
    # 流式回复段间间隔 <1s，8 秒可覆盖 LLM 思考间隙；工具消息不启动定时器，
    # 所以长工具调用期间状态行永远「运行中」，不会被误标完成。
    _CARD_SEAL_DELAY = 8.0

    # 容量保护阈值（飞书 interactive 请求体上限 ~30KB / JSON 2.0 元素上限 200）：
    # 同一张卡追加次数 / 块数 / 整卡 JSON 字节数任一超限 → 主动开新卡，
    # 避免无限追加触达卡片上限后 PATCH 被拒、旧内容断掉。
    _CARD_MAX_APPENDS = 30
    _CARD_MAX_BLOCKS = 60
    _CARD_MAX_BYTES = 15000

    @staticmethod
    def _card_exceeds_limits(card: dict) -> bool:
        """卡片容量保护：追加次数 / 块数 / 字节数任一超限 → 应开新卡。"""
        if card.get("append_count", 0) >= FeishuAdapter._CARD_MAX_APPENDS:
            return True
        if len(card.get("blocks", [])) >= FeishuAdapter._CARD_MAX_BLOCKS:
            return True
        try:
            if len(str(card.get("content", "")).encode("utf-8")) >= FeishuAdapter._CARD_MAX_BYTES:
                return True
        except Exception:
            pass
        return False
    ...
    def _is_closing_message(content: str) -> bool:
        """收尾类消息：记忆保存/后台审查通知（💾 开头）或 memory 工具调用。

        回复完成后 Hermes 会调 memory 工具收尾，并可能发「💾 Memory updated」
        通知——它们不该被当成任务工具（会把「✅ 完成」打回「运行中」），
        也不该作为正文追加到卡片上。
        """
        text = content.strip()
        if not text:
            return False
        if text.startswith("\U0001F4BE"):  # 💾 记忆通知
            return True
        first_line = text.split("\n")[0]
        # verbose 工具格式：📝 memory(...) 或 memory(...)（记忆保存是收尾动作，非任务工具）
        if re.search(r"\bmemory\s*\(", first_line):
            return True
        return False

    def _cancel_seal_timer(self, chat_id: str) -> None:
        """取消该会话的密封定时器（工具消息/新一轮活动时调用）。"""
        timers = getattr(self, "_card_seal_timers", None)
        if timers is None:
            return
    ...
    def _schedule_seal(self, chat_id: str, card: dict) -> None:
        """重置密封定时器：N 秒后无新消息则把状态行改为「✅ 完成」。"""
        self._cancel_seal_timer(chat_id)
        if getattr(self, "_card_seal_timers", None) is None:
            return
        ...
        try:
            ...
            return
        task = loop.create_task(self._seal_card_later(chat_id, message_id))
        self._card_seal_timers[chat_id] = task

    def _get_patch_lock(self, chat_id: str) -> asyncio.Lock:
        """每会话一个 PATCH 锁（串行化卡片更新，防并发 400 跳变）。"""
        locks = getattr(self, "_card_patch_locks", None)
        if locks is None:
            locks = {}
            self._card_patch_locks = locks
    ...
    async def _seal_card_later(self, chat_id: str, message_id: str) -> None:
        """定时器触发：卡片仍是当前卡且无新消息 → PATCH「✅ 完成」。"""
        try:
            await asyncio.sleep(self._CARD_SEAL_DELAY)
            card = self._chat_card_map.get(chat_id)
            if not card or card.get("message_id") != message_id:
                return  # 卡片已换（新卡/入站清 map），不密封旧卡
            self._card_seal_timers.pop(chat_id, None)
            blocks = card.get("blocks", [])
            payload = FeishuAdapter._card_with_blocks(blocks, status="sealed")
            # 持锁 PATCH：与 send() 的 PATCH 串行，防并发 400 导致内容跳变
            async with self._get_patch_lock(chat_id):
                await self._patch_card_raw(message_id, payload)
        except asyncio.CancelledError:
            pass
        except Exception as exc:
            logger.warning("[Feishu] Card seal failed: %s", exc)

    @staticmethod
    def _looks_like_tool_message(content: str) -> bool:
        """识别 Hermes 工具进度消息（emoji + 动词/工具名开头）。"""
        text = content.strip()
        if not text:
            return False
        # terminal 代码块
        if FeishuAdapter._TERMINAL_MSG_RE.match(text):
            return True
        # 非工具 emoji 前缀（✅/💬/📎 等）→ 不是工具
        if FeishuAdapter._NON_TOOL_PREFIX.match(text):
            return False
        # emoji 开头
        m = FeishuAdapter._TOOL_MSG_RE.match(text)
        if not m:
            return False
        first_line = text.split("\n")[0]
        # verbose 模式：首行 tool_name(args) → 工具（完整调用详情）
        if re.search(r"\b\w+\(.*\)", first_line) and "(" in first_line:
            return True
        # 全角/半角冒号、省略号 → 工具（预览形式：动词: 参数）
    ...
    @staticmethod
    def _tool_block(content: str, expanded: bool = True) -> dict:
        """工具消息 → 折叠面板块。"""
    ...
    _CODE_MAX_LINES = 8    # 超过 8 行（或 600 字符）的代码块 → 折叠框收起
    _CODE_MAX_CHARS = 600

    @staticmethod
    def _split_message_elements(text: str) -> list:
        """消息文本 → 元素列表：长代码块拆成默认收起的折叠框（点开看全文）。

        CM 2026-08-07 要求：Hermes 回复里的代码框太长，改成缩短型显示。
        短代码块（≤8 行）保持 markdown 原样渲染，不打扰阅读。
        """
        # 流式片段修补（P-15）：Hermes 流式输出把代码块切成多段，开/闭围栏
        # 不在同一段 → 正则匹配不到完整代码块 → 未闭合围栏被飞书渲染错乱
        # （第二行跑到折叠框外）。段内 ``` 为奇数时补全成独立完整代码块：
        # 以 ``` 结尾（代码块结束段）→ 段首补开围栏；其余（开始段/中间段）
        # → 段尾补闭合围栏。段段自成一体，视觉连续、样式统一。
        if text.count("```") % 2 == 1:
        ...
        elements = []
        pos = 0
        for m in FeishuAdapter._CODE_FENCE_RE.finditer(text):
            ...
            lines = code.count("\n") + 1
            if lines > FeishuAdapter._CODE_MAX_LINES or len(code) > FeishuAdapter._CODE_MAX_CHARS:
                elements.append({
                    "tag": "collapsible_panel",
                    "expanded": False,
                    "background_color": "grey-50",
                    "border": {"color": "grey", "corner_radius": "8px"},
                    "header": {"title": {"tag": "plain_text", "content": f"{lang} · {lines} 行 · 点击展开"}},
    ...
    @staticmethod
    def _card_with_blocks(blocks: list, status: str = "running") -> str:
        """blocks → JSON 2.0 卡片（含状态行）。"""
        elements = []
        for b in blocks:
            if b["type"] == "message":
                elements.extend(FeishuAdapter._split_message_elements(b["text"]))
            elif b["type"] == "tools":
                elements.append(FeishuAdapter._tool_block(b["content"], expanded=b.get("expanded", True)))
        if status == "running":
            elements.append({"tag": "markdown", "content": "_⏳ 运行中…_"})
        elif status == "typing":
            elements.append({"tag": "markdown", "content": "_✍️ 回复中…_"})
        else:
            elements.append({"tag": "markdown", "content": "_✅ 完成_"})
        card = {
            "schema": "2.0",
            "config": {"wide_screen_mode": True},
    ...
    async def _patch_card_raw(self, message_id: str, payload: str) -> Any:
        """直接 PATCH 更新卡片（绕过 edit_message 的重建逻辑，保留 blocks 结构）。"""
        from lark_oapi.api.im.v1.model.patch_message_request import PatchMessageRequest
        from lark_oapi.api.im.v1.model.patch_message_request_body import PatchMessageRequestBody
        patch_body = PatchMessageRequestBody.builder().content(payload).build()
        patch_request = (
    ...
    def _build_outbound_payload(
        ...
        # 飞书卡片回复模式(用户要求):所有出站回复 → JSON 2.0 交互卡片。
