---
title: "Hermes 飞书卡片模式升级指南 / Feishu Card Mode Upgrade Guide"
date: 2026-08-04
type: 教程
...
# Hermes 飞书卡片模式升级指南 / Feishu Card Mode Upgrade Guide

> **升级任务书**：你的 Hermes 默认用「文本/富文本」回复飞书消息。本指南教你把它升级为「**交互卡片模式**」——回复变成一张可原地更新的卡片，过程消息全部保留。
>
> This guide upgrades Hermes' Feishu replies to interactive card mode: replies become one card that updates in place, and every progress message is kept.
>
> 全程实战验证通过（2026-08-04）：发送卡片 ✅、原地更新 ✅、过程消息 10/10 全保留 ✅。Field-tested end to end — every pitfall below was hit for real.
...
## 三、升级步骤(9 步) / The 9 Steps

> 以下步骤由 Agent 执行。所有文件操作前先备份。Back up before any file change.

### 步骤 1:备份适配器 / Backup the adapter
...
### 步骤 2:修改回复格式 → JSON 2.0 卡片 / Reply format → JSON 2.0 card

找到 `_build_outbound_payload` 方法,把它的返回值从 `post`/`text` 改为 `interactive` 卡片。Change the return of `_build_outbound_payload` from `post`/`text` to an `interactive` card:
...
**⚠️ 为什么必须 JSON 2.0(`"schema": "2.0"`)?** 旧版卡片(JSON 1.0)的 markdown 组件**不支持表格**。只有 2.0 支持标准 markdown(含表格、代码块、分割线)。The JSON 1.0 markdown component does not support tables — 2.0 is required.

### 步骤 3:加回退链(interactive → post → text) / Fallback chain

在 `send()` 方法中,发送失败时的回退逻辑改为三级回退,保证极端情况消息必达。Three-level fallback so messages always get through:
...
(API 响应层失败时同样处理:`interactive` 失败 → 重发 `post`;`post` 失败 → 重发 `text`。Handle the API-response layer the same way.)

### 步骤 4:修改 `edit_message` → 卡片更新必须走 PATCH / Card updates must use PATCH

**这是最容易踩的坑**:Hermes SDK 的 `message.update` 走的是 **PUT** 接口,而 PUT **不接受** `interactive`(实测报 `[230001] invalid msg_type`)。卡片原地更新**必须走 PATCH**。The SDK's `message.update` uses PUT, which rejects `interactive` — in-place card updates must use PATCH:
...
> 注意:`PatchMessageRequestBody` **只有 `content` 字段**(实测传 `msg_type` 会报 `[230001] invalid msg_type` / `[99992402] field validation failed`)。It takes `content` only.

### 步骤 5:加「会话 → 卡片」映射(同一轮内追加) / Chat → card map

在 `__init__` 中初始化(纯内存,不持久化)。In-memory only — after a restart the map is empty, so a new card is always sent and stale content never overwrites an old card:
...
在 `send()` 开头加追加逻辑(同一轮内多条消息 → 追加到同一张卡片,不覆盖)。Append (never overwrite) at the start of `send()`:
...
在 `send()` 结尾记录新卡。Record the new card at the end of `send()`:
...
在收到用户入站消息处(如 `_dispatch_inbound_event`)清掉映射 = 翻页(用户新消息 = 新轮次 → 发新卡)。A new inbound message clears the map — page turn, new card:
...
### 步骤 6:配置工具进度分组 = separate(关键!) / Tool progress grouping = separate

**这是"过程消息被覆盖消失"的根因**:Hermes 默认 `tool_progress_grouping = accumulate`,会把多条工具消息**编辑到同一个气泡、后一条覆盖前一条**(实测:10 条消息最后只剩 2、3)。This is the root cause of "progress messages disappearing" — `accumulate` edits multiple tool messages into one bubble, each overwriting the previous (10 messages ended with only 2–3 left):

```bash
hermes config set display.platforms.feishu.tool_progress_grouping "separate"
```

> `separate` = 每个工具一条独立消息,互不覆盖,全部保留。One independent message per tool — nothing overwritten, everything kept.

### 步骤 7:配置不清理过程消息 / Don't clean up progress messages

```bash
hermes config set display.platforms.feishu.cleanup_progress "false"
```

> `cleanup_progress: true` 时,工具过程消息会在最终回复后**被删除**,只剩结果。用户要求看到所有过程 → 必须关掉。When `true`, progress messages are deleted after the final reply — turn it off to keep them.

### 步骤 8:重启 gateway / Restart the gateway
...
> ⚠️ gateway 进程**不能从内部重启自己**(信号传播会杀死自身)。用计划任务/外部 shell 执行,或直接发 `/restart`。The gateway cannot restart itself from inside — use an external shell or send `/restart`.

### 步骤 9:验证 / Verify

发一句话给 bot,让它连续执行几次工具调用(如 `echo "消息 1"` ~ `echo "消息 10"`),确认。Send a message that triggers several tool calls and confirm:

- [ ] 收到的是**卡片**(不是纯文本)。You receive a card, not plain text.
- [ ] 同一轮内 10 条消息**全部保留**,追加在同一张卡上。All 10 messages kept, appended to one card.
...
位于本仓库 `scripts/feishu_card.py`(可复制到任意目录)。Send / update cards manually — zero dependencies, stdlib only:
...
核心机制(三行 API,记住就够)。Core mechanism — three API calls:
...
> ⚠️ `content` 字段是整个卡片 JSON 的**字符串**(要 `json.dumps` 转义)。`content` must be the whole card JSON as a string.

### 脚本 2:`apply_feishu_card_patch.py` —— 一键重打补丁

`hermes update` 会覆盖 `adapter.py`,卡片功能会失效。运行。`hermes update` overwrites `adapter.py` — run this to restore card mode:

```bash
python apply_feishu_card_patch.py
```

幂等:已打补丁会跳过;每次应用前自动备份 `adapter.py.bak-feishu-card-<时间戳>`。包含全部补丁(步骤 2~5)。Idempotent — skips when already patched, auto-backs up before each apply. Contains all patches (steps 2–5).
...
## 六、验证清单(交付前自检) / Verification Checklist
...
## 七、直接把本文档交给 Agent 执行 / Hand This Document to Your Agent

如果你不想手动操作,直接把本文档(二~五节)发给你的 Hermes,说。If you'd rather not do it manually, send sections 2–5 of this document to your Hermes and say:

> **「请按这份文档,把我的飞书回复升级为卡片模式。」** "Please follow this document and upgrade my Feishu replies to card mode."

Agent 会:备份 → 打补丁(步骤 2~5)→ 改配置(步骤 6~7)→ 提示重启 → 验证。全程可回退(备份文件在 `adapter.py.bak-card`)。

---

*本指南由作者在 Hermes 上实测产出(2026-08-04),所有坑都踩过一遍,照做即可。Field-tested 2026-08-04 — every pitfall here was hit for real.*
